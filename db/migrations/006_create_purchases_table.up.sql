...
-- Description: Course purchase records and Razorpay integrations
-- ============================================================
...
CREATE TABLE purchases (
    ...
    -- Valid time window
    CONSTRAINT chk_purchases_valid_window CHECK (valid_until > valid_from),
...
-- 2. Course buyers (Admin analytics)
CREATE INDEX idx_purchases_course_id
...
-- ── Comments ─────────────────────────────────────────────────
COMMENT ON TABLE  purchases                     IS 'Course purchase records — Razorpay payments';
COMMENT ON COLUMN purchases.razorpay_order_id   IS 'Razorpay Order ID (idempotency key)';
COMMENT ON COLUMN purchases.razorpay_payment_id IS 'Razorpay Payment ID (set after successful payment)';
COMMENT ON COLUMN purchases.razorpay_signature  IS 'HMAC signature for payment verification';
COMMENT ON COLUMN purchases.amount_paid         IS 'Snapshot of course price at time of purchase';
COMMENT ON COLUMN purchases.currency            IS 'ISO 4217 currency code (e.g., INR, USD)';
COMMENT ON COLUMN purchases.metadata            IS 'Flexible JSON storage (webhooks, errors, coupons)';
COMMENT ON COLUMN purchases.valid_from          IS 'Access start time';
COMMENT ON COLUMN purchases.valid_until         IS 'Access end time (valid_from + course validity)';