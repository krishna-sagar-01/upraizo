...
func SetupRoutes(app *fiber.App, deps *AppDependencies) {
	...
	authLevel := auth.Group("/", middleware.AuthRequired(deps.JWTManager, deps.SessionRepo))
	...
	adminAuthPublic.Post("/logout", deps.AdminAuthHdl.Logout)
	adminAuthPublic.Post("/refresh", deps.AdminAuthHdl.Refresh)
	...
	// ADMIN ROUTES EBOOK
	// =============================
	adminEbooks := admin.Group("/ebooks")
	adminEbooks.Get("/", deps.EbookHdl.AdminList)
	adminEbooks.Post("/", deps.EbookHdl.Create)
	adminEbooks.Get("/:id", deps.EbookHdl.GetByID)
	adminEbooks.Put("/:id", deps.EbookHdl.Update)
	adminEbooks.Delete("/:id", deps.EbookHdl.Delete)
	adminEbooks.Post("/:id/file", deps.EbookHdl.UploadFile)
	adminEbooks.Post("/:id/thumbnail", deps.EbookHdl.UploadThumbnail)
	...
	adminLessonOps.Get("/video-presigned-url", deps.LessonHdl.GetVideoUploadURL)
	adminLessonOps.Post("/video-complete", deps.LessonHdl.CompleteVideoUpload)
	...
	adminTickets.Delete("/:id", deps.TicketHdl.AdminDeleteTicket)

	// ==========================================
	// USER PROTECTED ROUTES
	// ==========================================
	user := api.Group("/user", middleware.AuthRequired(deps.JWTManager, deps.SessionRepo))
	...
	support := api.Group("/support", middleware.AuthRequired(deps.JWTManager, deps.SessionRepo))
	...
	courses.Get("/:slug/curriculum", middleware.OptionalAuth(deps.JWTManager, deps.SessionRepo), deps.CourseHdl.GetCurriculum)

	// ==========================================
	// PUBLIC EBOOK ROUTES
	// ==========================================
	ebooks := api.Group("/ebooks")
	ebooks.Get("/", deps.EbookHdl.List)
	ebooks.Get("/my", middleware.AuthRequired(deps.JWTManager, deps.SessionRepo), deps.EbookHdl.GetMyEbooks)
	ebooks.Get("/:slug", middleware.OptionalAuth(deps.JWTManager, deps.SessionRepo), deps.EbookHdl.GetBySlug)

	// ==========================================
	// PAYMENT ROUTES
	// ==========================================
	payment := api.Group("/payment")
	payment.Post("/orders", middleware.AuthRequired(deps.JWTManager, deps.SessionRepo), deps.PurchaseHdl.CreateOrder)
	payment.Get("/my", middleware.AuthRequired(deps.JWTManager, deps.SessionRepo), deps.PurchaseHdl.GetMyPayments)
	payment.Get("/:id", middleware.AuthRequired(deps.JWTManager, deps.SessionRepo), deps.PurchaseHdl.GetPayment)
