...
func (s *UserService) UpdateProfile(ctx context.Context, userID uuid.UUID, req *dto.UpdateProfileRequest) (dto.SafeUser, error) {
	...
	if updated {
		// Use consolidated update if both name and preferences changed, 
		// otherwise use specific optimized methods.
		nameChanged := req.Name != nil
		prefsChanged := req.Theme != nil || req.Notifications != nil

		if nameChanged && prefsChanged {
			if err := s.userRepo.UpdateProfile(ctx, userID, user.Name, user.Preferences); err != nil {
				return dto.SafeUser{}, utils.Internal(err)
			}
		} else if nameChanged {
