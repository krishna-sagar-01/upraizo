...
func (r *TicketRepository) scanMessage(row pgx.Row) (*models.TicketMessage, error) {
	...
	err := row.Scan(
		...
		&tm.Message, &tm.Attachments, &tm.CreatedAt,
	)
	return tm, err
}
...
// GetUserTickets: User ke saare tickets list karne ke liye
...
func (r *TicketRepository) AddMessage(ctx context.Context, tm *models.TicketMessage) error {
	...
	query := `
		INSERT INTO ticket_messages (id, ticket_id, user_id, admin_id, message, attachments, created_at)
	...
	_, err := r.db.Exec(writeCtx, query,
		tm.ID, tm.TicketID, tm.UserID, tm.AdminID, tm.Message, tm.Attachments, tm.CreatedAt,
	)

	// Ticket ki 'updated_at' clock ko update karo taaki wo list mein upar dikhe
	if err == nil {
		_ = r.UpdateStatus(ctx, tm.TicketID, models.TicketStatusInProgress)
	}

	return err
}

// UpdateMessageAttachments: Worker use karega URLs update karne ke liye
func (r *TicketRepository) UpdateMessageAttachments(ctx context.Context, id uuid.UUID, attachments models.TicketAttachments) error {
	query := `UPDATE ticket_messages SET attachments = $2 WHERE id = $1`
	_, err := r.db.Exec(ctx, query, id, attachments)
	return err
}
...
func (r *TicketRepository) GetConversation(ctx context.Context, ticketID uuid.UUID) ([]*models.TicketMessage, error) {
	query := `SELECT id, ticket_id, user_id, admin_id, message, attachments, created_at 
