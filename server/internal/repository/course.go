...
// Update: Consolidated update for all course fields
func (r *CourseRepository) Update(ctx context.Context, c *models.Course) error {
	query := `
		UPDATE courses SET 
			title = $2, slug = $3, description = $4, thumbnail_url = $5, 
			category = $6, price = $7, original_price = $8, discount_label = $9, 
			discount_expires_at = $10, validity_days = $11, is_published = $12, 
			razorpay_item_id = $13, updated_at = $14 
		WHERE id = $1`

	return r.performUpdate(query, "full update", c.ID,
		c.ID, c.Title, c.Slug, c.Description, c.ThumbnailURL, c.Category,
		c.Price, c.OriginalPrice, c.DiscountLabel, c.DiscountExpiresAt,
		c.ValidityDays, c.IsPublished, c.RazorpayItemID, time.Now())
}
