...
// UpdateProfile updates both name and preferences in a single transaction.
func (r *UserRepository) UpdateProfile(ctx context.Context, userID uuid.UUID, name string, prefs models.UserPreferences) error {
	query := `UPDATE users SET name = $2, preferences = $3, updated_at = $4 WHERE id = $1`
	return r.performUpdate(query, "consolidated profile update", userID, userID, name, prefs, time.Now())
}
