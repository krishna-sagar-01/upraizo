...
// TicketAttachments maps to the attachments JSONB array.
// Stores an array of file URLs (e.g., Cloudflare R2 keys or S3 URLs).
type TicketAttachments []string
...
type TicketMessage struct {
	...
	Attachments TicketAttachments `json:"attachments"`
	CreatedAt   time.Time         `json:"created_at"`
}
...
// HasContent ensures the message has either text or attachments, reflecting
// the database constraint: chk_ticket_messages_content.
func (tm *TicketMessage) HasContent() bool {
	return len(tm.Message) > 0 || len(tm.Attachments) > 0
}
