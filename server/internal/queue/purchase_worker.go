...
func (w *PurchaseWorker) process(d amqp.Delivery) {
	...
	purchase, err := w.purchaseRepo.GetByID(workerCtx, uid)
	if err != nil {
	...
	if purchase.CourseID != nil {
		course, err := w.courseRepo.GetByID(workerCtx, *purchase.CourseID)
		if err != nil {
	...
	err = w.purchaseRepo.ActivateAccess(workerCtx, purchase.ID, validityDays)
	if err != nil {
