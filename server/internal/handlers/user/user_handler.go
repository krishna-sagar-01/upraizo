...
func (h *UserHandler) UploadAvatar(c *fiber.Ctx) error {
	...
	// 5. Process Asynchronously (Queue for worker)
	if err := h.userService.QueueAvatarUpdate(c.Context(), userID, localPath); err != nil {
		return err
	}

	return c.Status(http.StatusOK).JSON(fiber.Map{
		"success": true,
		"message": "Avatar upload initiated. It will be updated shortly.",
	})
}
