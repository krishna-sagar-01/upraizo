...
func (h *CourseHandler) GetCurriculum(c *fiber.Ctx) error {
	...
	// Try to get user_id from locals (populated by AuthRequired middleware if present)
	...
	res, err := h.svc.GetCurriculum(c.Context(), slug, userIDPtr)
	if err != nil {
		return err
	}
...
func (h *CourseHandler) Update(c *fiber.Ctx) error {
	...
	if err := c.BodyParser(&req); err != nil {
		utils.Error("Admin Course Update: Body parsing failed", err, map[string]any{"id": id})
		return utils.BadRequest("Invalid request body")
	}
